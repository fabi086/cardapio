      ...
      {totalItems > 0 && !isCartOpen && (
        <>
          <div className="fixed bottom-4 left-4 right-4 md:hidden z-40">
            <button onClick={() => setIsCartOpen(true)} className={`w-full bg-italian-green text-white py-3 px-6 rounded-xl shadow-xl flex items-center justify-between animate-in slide-in-from-bottom-4 ${isCartAnimating ? 'scale-105 bg-green-600' : ''}`} style={{ backgroundColor: 'var(--cart-bg)' }}>
              ...
              <div className={`flex items-center gap-2 bg-black/20 px-3 py-1.5 rounded-lg ${isCartAnimating ? 'scale-110' : ''}`}><ShoppingBag className="w-5 h-5" /><span className="font-bold">{totalItems}</span></div>
            </button>
          </div>
          <div className="hidden md:block fixed bottom-8 right-8 z-40">
            <button onClick={() => setIsCartOpen(true)} className={`bg-italian-green text-white w-16 h-16 rounded-full shadow-xl flex items-center justify-center hover:scale-110 group ${isCartAnimating ? 'scale-125 bg-green-600' : ''}`} style={{ backgroundColor: 'var(--cart-bg)' }}>
