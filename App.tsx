...
const checkStoreOpenAdvanced = (schedule?: WeeklySchedule, timezone?: string): { isOpen: boolean, message: string } => {
    ...
    try {
        ...
        if (isOpenNow) return { isOpen: true, message: 'Aberto Agora' };
        else {
           ...
           if (nextInterval) return { isOpen: false, message: `Fechado. Abre às ${nextInterval.start}` };
           else return { isOpen: false, message: 'Fechado por hoje' };
        }
    } catch (e) {
...
function App() {
  ...
  useEffect(() => {
    ...
    if (storeSettings.colors) {
      ...
      root.style.setProperty('--btn-primary', storeSettings.colors.buttons || storeSettings.colors.primary);
      root.style.setProperty('--cart-bg', storeSettings.colors.cart || storeSettings.colors.secondary || '#008C45');
  ...
  useEffect(() => {
    ...
    if (link) { link.href = faviconUrl; } else { const newLink = document.createElement('link'); newLink.rel = 'icon'; newLink.href = faviconUrl; document.head.appendChild(newLink); }
  }, [storeSettings.faviconUrl]);

  useEffect(() => {
    const pageTitle = storeSettings.seoTitle?.trim() ? storeSettings.seoTitle : (storeSettings.name ? storeSettings.name : 'Cardápio Digital');
    document.title = pageTitle;
    const updateMetaTag = (selector: string, content: string) => { if (!content) return; let element = document.querySelector(selector); if (!element) { element = document.createElement('meta'); if (selector.startsWith('meta[property')) { const property = selector.replace("meta[property='", "").replace("']", ""); element.setAttribute('property', property); } else if (selector.startsWith('meta[name')) { const name = selector.replace("meta[name='", "").replace("']", ""); element.setAttribute('name', name); } document.head.appendChild(element); } element.setAttribute('content', content); };
    updateMetaTag("meta[property='og:title']", pageTitle);
    updateMetaTag("meta[property='og:description']", storeSettings.seoDescription || '');
    if (storeSettings.seoBannerUrl) { updateMetaTag("meta[property='og:image']", storeSettings.seoBannerUrl); }
  ...
  useEffect(() => {
    if (isDarkMode) { document.documentElement.classList.add('dark'); localStorage.setItem('theme', 'dark'); } else { document.documentElement.classList.remove('dark'); localStorage.setItem('theme', 'light'); }
  }, [isDarkMode]);
  const toggleTheme = () => setIsDarkMode(!isDarkMode);

  useEffect(() => {
     if (storeSettings.schedule) { setStoreStatus(checkStoreOpenAdvanced(storeSettings.schedule, storeSettings.timezone)); } else if (storeSettings.openingHours) { setStoreStatus({ isOpen: true, message: '' }); }
     if (storeSettings.enableGuide !== false) { const hasSeenGuide = localStorage.getItem('hasSeenGuide_v1'); if (!hasSeenGuide) { setTimeout(() => setShowGuide(true), 1500); } }
  }, [storeSettings]);
  const closeGuide = () => { setShowGuide(false); localStorage.setItem('hasSeenGuide_v1', 'true'); };
  const restartGuide = () => { setShowGuide(true); };

  const openPizzaBuilder = (categoryId: string, firstHalf: Product | null = null) => { setPizzaBuilderCategory(categoryId); setPizzaBuilderFirstHalf(firstHalf); setIsPizzaBuilderOpen(true); };
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (settingsData) {
        ...
        const mergedColors = { ...DEFAULT_SETTINGS.colors, ...colors }; if (!mergedColors.modes) mergedColors.modes = DEFAULT_SETTINGS.colors?.modes;
        ...
        setStoreSettings({
            ...
            enableTableOrder: settingsData.enable_table_order ?? true,
  ...
  useEffect(() => { if (menuData.length > 0 && !activeCategory) { const firstContentCategory = menuData.find(c => c.id !== 'promocoes') || menuData[0]; if (firstContentCategory) setActiveCategory(firstContentCategory.id); } }, [menuData]);
  useEffect(() => { const handleScroll = () => { if (window.scrollY > 300) setShowScrollTop(true); else setShowScrollTop(false); }; const observer = new IntersectionObserver((entries) => { if (isManualScrolling.current) return; const visibleEntries = entries.filter(e => e.isIntersecting).sort((a, b) => b.intersectionRatio - a.intersectionRatio); if (visibleEntries.length > 0) { const targetId = visibleEntries[0].target.id.replace('category-', ''); setActiveCategory(targetId); } }, { rootMargin: '-120px 0px -50% 0px', threshold: [0, 0.1, 0.5] }); menuData.forEach(cat => { const el = document.getElementById(`category-${cat.id}`); if (el) observer.observe(el); }); window.addEventListener('scroll', handleScroll); return () => { observer.disconnect(); window.removeEventListener('scroll', handleScroll); }; }, [menuData]);

  const scrollToTop = () => { window.scrollTo({ top: 0, behavior: 'smooth' }); };
  useEffect(() => { try { localStorage.setItem('spagnolli_cart', JSON.stringify(cartItems)); } catch (error) { console.error("Failed to save cart:", error); } }, [cartItems]);
  useEffect(() => { if (showToast) { const timer = setTimeout(() => setShowToast(false), 3000); return () => clearTimeout(timer); } }, [showToast]);

  const handleUpdateProduct = async (catId: string, pId: number, updates: Partial<Product>) => { setMenuData(prev => prev.map(c => { return { ...c, items: c.items.map(p => p.id === pId ? {...p, ...updates} : p) }; })); if (supabase) { const payload: any = { ...updates }; if (updates.options) payload.options = JSON.stringify(updates.options); if (updates.tags) payload.tags = updates.tags; if (updates.additional_categories) payload.additional_categories = JSON.stringify(updates.additional_categories); await supabase.from('products').update(payload).eq('id', pId); fetchData(); } };
  const handleAddProduct = async (catId: string, product: Omit<Product, 'id'>) => { const tempId = Date.now(); setMenuData(prev => prev.map(c => { if(c.id === catId) return { ...c, items: [...c.items, { ...product, id: tempId, category: catId }]}; return c; })); if(supabase) { const payload: any = { ...product, category_id: catId, options: JSON.stringify(product.options || []), tags: product.tags || [], additional_categories: JSON.stringify(product.additional_categories || []) }; await supabase.from('products').insert([payload]); fetchData(); } };
  const handleDeleteProduct = async (catId: string, pId: number) => { setMenuData(prev => prev.map(c => { return { ...c, items: c.items.filter(p => p.id !== pId) }; })); if(supabase) await supabase.from('products').delete().eq('id', pId); };
  ...
  const addToCart = (product: Product, quantity: number = 1, observation: string = '', selectedOptions?: CartItem['selectedOptions']) => {
    ...
    setCartItems(prev => {
      const existingItemIndex = prev.findIndex(item => { const itemOptionsKey = item.selectedOptions ? JSON.stringify(item.selectedOptions.sort((a,b) => a.choiceName.localeCompare(b.choiceName))) : ''; return item.id === product.id && (item.observation || '').trim() === normalizedObservation && itemOptionsKey === optionsKey; });
      if (existingItemIndex >= 0) { const newItems = [...prev]; newItems[existingItemIndex] = { ...newItems[existingItemIndex], quantity: newItems[existingItemIndex].quantity + quantity }; return newItems; } else { return [...prev, { ...product, quantity, observation: normalizedObservation, selectedOptions }]; }
    });
    setShowToast(true); setIsCartAnimating(true); setTimeout(() => setIsCartAnimating(false), 300);
  };

  const updateQuantity = (index: number, newQuantity: number) => { if (newQuantity < 1) { if (window.confirm('Remover item?')) removeFromCart(index); return; } setCartItems(prev => { const n = [...prev]; n[index].quantity = newQuantity; return n; }); };
  const updateObservation = (index: number, newObs: string) => { setCartItems(prev => { const n = [...prev]; n[index].observation = newObs; return n; }); };
  ...
  const handleCategorySelect = (id: string) => { isManualScrolling.current = true; setActiveCategory(id); const element = document.getElementById(`category-${id}`); if (element) { const headerOffset = 180; const elementPosition = element.getBoundingClientRect().top; const offsetPosition = elementPosition + window.scrollY - headerOffset; window.scrollTo({ top: offsetPosition, behavior: "smooth" }); setTimeout(() => { isManualScrolling.current = false; }, 800); } };

  const totalItems = cartItems.reduce((acc, item) => acc + (item.quantity || 0), 0);
  const totalPrice = cartItems.reduce((acc, item) => { const optionsPrice = item.selectedOptions ? item.selectedOptions.reduce((s,o) => s + o.price, 0) : 0; return acc + ((item.price + optionsPrice) * (item.quantity || 0)); }, 0);
  ...
  const displayCategories = useMemo(() => {
    return menuData.map(cat => {
        const filteredItems = cat.items.filter(item => {
           if (activeTags.length > 0) { const itemTags = item.tags || []; const hasAllTags = activeTags.every(tag => itemTags.includes(tag)); if (!hasAllTags) return false; }
           if (searchTerm) { const normalize = (str: string) => str.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, ""); const searchTerms = normalize(searchTerm).split(/\s+/).filter(t => t.length > 0); if (searchTerms.length === 0) return true; const itemSearchableText = normalize([item.name, item.description || '', item.code || '', item.subcategory || '', (item.ingredients || []).join(' '), cat.name].join(' ')); return searchTerms.every(term => itemSearchableText.includes(term)); }
           return true;
        });
        return { ...cat, items: filteredItems };
      }).filter(cat => { if (searchScope !== 'all' && cat.id !== searchScope) return false; if (cat.id === 'promocoes' && !searchTerm && activeTags.length === 0) return false; return true; });
  }, [menuData, searchTerm, searchScope, activeTags]);

  const toggleTag = (tagId: string) => { setActiveTags(prev => prev.includes(tagId) ? prev.filter(t => t !== tagId) : [...prev, tagId]); };
  ...
  return (
    <div className="min-h-screen pb-24 md:pb-0 font-sans transition-colors duration-300" style={{ backgroundColor: 'var(--bg-body, #f5f5f4)', color: 'var(--text-body, #292524)' }}>
      ...
      {tableNumber && ( <div className="bg-italian-green text-white px-4 py-2 text-center text-sm font-bold sticky top-0 z-[60] shadow-md flex items-center justify-center gap-2 animate-in slide-in-from-top border-b border-green-700"> <div className="bg-white text-italian-green rounded-full w-6 h-6 flex items-center justify-center text-xs shadow-sm font-extrabold">{tableNumber}</div> Você está pedindo na Mesa {tableNumber} </div> )}
      {!storeStatus.isOpen && !tableNumber && ( <div className="bg-red-600 text-white px-4 py-2 text-center text-sm font-bold sticky top-0 z-50"><Clock className="w-4 h-4 inline mr-2" /> {storeStatus.message}</div> )}

      <Header cartCount={totalItems} onOpenCart={() => setIsCartOpen(true)} animateCart={isCartAnimating} storeName={storeSettings.name} logoUrl={storeSettings.logoUrl} isDarkMode={isDarkMode} onToggleTheme={toggleTheme} whatsapp={storeSettings.whatsapp} phone={storeSettings.phones[0] || ''} onOpenInfo={() => setIsInfoModalOpen(true)} isOpenNow={storeStatus.isOpen} onOpenTracker={() => setIsTrackerOpen(true)} onOpenAI={() => setIsAIOpen(true)}/>
      ...
      <main className="max-w-5xl mx-auto px-4 pt-6">
        ...
        <div className="space-y-10">
          {displayCategories.map((category) => {
            ...
            if (hasSubcategories) { category.items.forEach(item => { if (item.subcategory) { if (!groupedItems[item.subcategory]) groupedItems[item.subcategory] = []; groupedItems[item.subcategory].push(item); } else ungroupedItems.push(item); }); }
            ...
            return (
              <section key={category.id} id={`category-${category.id}`} className="scroll-mt-64">
                ...
                {!searchTerm && activeTags.length === 0 && isPizzaCategory && ( <div className="mb-6"> <button onClick={() => openPizzaBuilder(category.id)} className="w-full bg-gradient-to-r from-italian-red to-red-700 text-white p-4 rounded-xl shadow-md flex items-center justify-between group hover:shadow-lg transition-all"> <div className="flex items-center gap-4"> <div className="bg-white/20 p-3 rounded-full"><PieChart className="w-8 h-8 text-white" /></div> <div className="text-left"><h3 className="font-bold text-lg leading-tight">Crie sua Pizza Meia a Meia</h3><p className="text-white/80 text-xs">Escolha 2 sabores diferentes</p></div> </div> <div className="bg-white text-italian-red px-4 py-2 rounded-full text-sm font-bold group-hover:scale-105 transition-transform">Montar Agora</div> </button> </div> )}
                {hasSubcategories ? ( <div className="space-y-6"> {ungroupedItems.length > 0 && ( <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4"> {ungroupedItems.map((product) => { const isFirst = !firstProductFound; if (isFirst) firstProductFound = true; return ( <ProductCard key={product.id} product={product} onAddToCart={addToCart} id={isFirst ? "tour-product-card" : undefined} allowHalfHalf={isPizzaCategory} onOpenPizzaBuilder={(p) => openPizzaBuilder(category.id, p)} currencySymbol={storeSettings.currencySymbol} /> ); })} </div> )} {Object.entries(groupedItems).map(([sub, products]) => ( <div key={sub}> <h3 className="text-lg font-bold text-stone-800 dark:text-stone-300 mb-3 ml-1 flex items-center gap-2"><span className="w-1.5 h-1.5 bg-stone-500 rounded-full"></span>{sub}</h3> <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4"> {products.map((product) => ( <ProductCard key={product.id} product={product} onAddToCart={addToCart} allowHalfHalf={isPizzaCategory} onOpenPizzaBuilder={(p) => openPizzaBuilder(category.id, p)} currencySymbol={storeSettings.currencySymbol} /> ))} </div> </div> ))} </div> ) : ( <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4"> {category.items.map((product) => { const isFirst = !firstProductFound; if (isFirst) firstProductFound = true; return ( <ProductCard key={product.id} product={product} onAddToCart={addToCart} id={isFirst ? "tour-product-card" : undefined} allowHalfHalf={isPizzaCategory} onOpenPizzaBuilder={(p) => openPizzaBuilder(category.id, p)} currencySymbol={storeSettings.currencySymbol} /> ); })} </div> )}
              </section>
            );
          })}
          {displayCategories.length === 0 && ( <div className="bg-stone-100 border border-stone-300 rounded-lg p-8 text-center dark:bg-stone-800 dark:border-stone-700"> <p className="text-stone-600 font-medium dark:text-stone-400 text-lg">Nenhum produto encontrado.</p> <p className="text-sm text-stone-500 mt-2">Tente buscar por outro termo ou remova os filtros.</p> </div> )}
        </div>
      </main>
      ...
      <AIAssistant 
        ...
        currencySymbol={storeSettings.currencySymbol || 'R$'} 
        cartItems={cartItems}
        onAddToCart={addToCart}
        storeSettings={storeSettings}
      />

      {showToast && ( <div className="fixed top-20 right-4 z-50 animate-in fade-in slide-in-from-right duration-300 pointer-events-none"> <div className="bg-italian-green text-white px-4 py-3 rounded-lg shadow-lg flex items-center gap-2"> <div className="bg-white/20 p-1 rounded-full"><Check className="w-4 h-4" /></div> <span className="font-medium">Item adicionado ao carrinho!</span> </div> </div> )}
      {totalItems > 0 && !isCartOpen && ( <> <div className="fixed bottom-4 left-4 right-4 md:hidden z-40"> <button onClick={() => setIsCartOpen(true)} className={`w-full bg-italian-green text-white py-3 px-6 rounded-xl shadow-xl flex items-center justify-between animate-in slide-in-from-bottom-4 ${isCartAnimating ? 'scale-105 bg-green-600' : ''}`} style={{ backgroundColor: 'var(--cart-bg)' }}> <div className="flex flex-col items-start text-left"><span className="text-xs font-light text-green-100">Total</span><span className="font-bold text-lg">{storeSettings.currencySymbol || 'R$'} {totalPrice.toFixed(2).replace('.', ',')}</span></div> <div className={`flex items-center gap-2 bg-black/20 px-3 py-1.5 rounded-lg ${isCartAnimating ? 'scale-110' : ''}`}><ShoppingBag className="w-5 h-5" /><span className="font-bold">{totalItems}</span></div> </button> </div> <div className="hidden md:block fixed bottom-8 right-8 z-40"> <button onClick={() => setIsCartOpen(true)} className={`bg-italian-green text-white w-16 h-16 rounded-full shadow-xl flex items-center justify-center hover:scale-110 group ${isCartAnimating ? 'scale-125 bg-green-600' : ''}`} style={{ backgroundColor: 'var(--cart-bg)' }}> <ShoppingBag className={`w-8 h-8 group-hover:animate-pulse ${isCartAnimating ? 'animate-bounce' : ''}`} /> <span className="absolute -top-2 -right-2 bg-italian-red text-white text-sm font-bold w-7 h-7 flex items-center justify-center rounded-full border-2 border-white dark:border-stone-800">{totalItems}</span> </button> </div> </> )}
    </div>
  );
}
