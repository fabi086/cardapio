...
import { X, Check, Search, CirclePercent, ChevronRight } from 'lucide-react';
...
export const PizzaBuilderModal: React.FC<PizzaBuilderModalProps> = ({
  ...
  const handleFinish = () => {
    ...
    // Construct a composite product
    const mixedPizza: Product = {
      id: parseInt(`${firstHalf.id}${secondHalf.id}`), // Synthetic ID
      name: `Meia ${firstHalf.name} / Meia ${secondHalf.name}`,
      description: `1/2 ${firstHalf.name} (${firstHalf.description})\n1/2 ${secondHalf.name} (${secondHalf.description})`,
      price: basePrice,
        ...
        <div className="flex-1 overflow-y-auto p-4">
          ...
          {step === 3 && firstHalf && secondHalf && (
            <div className="space-y-6 animate-in fade-in">
              ...
              {/* Options (Use options from first pizza assuming structure is same) */}
