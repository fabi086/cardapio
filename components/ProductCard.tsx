...
export const ProductCard: React.FC<ProductCardProps> = ({ product, onAddToCart }) => {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-stone-100 overflow-hidden flex flex-col h-full hover:shadow-md transition-shadow">
      {/* Product Image */}
      <div 
        className="h-40 w-full bg-stone-100 relative overflow-hidden group cursor-pointer"
      ...
      <div className="p-4 flex-1 flex flex-col">
        ...
        {hasOptions && !isExpanded && (
           <div className="mt-auto mb-3">
             <span className="text-[10px] bg-yellow-50 text-yellow-700 px-2 py-1 rounded border border-yellow-100 font-semibold">
               Personalizável
             </span>
           </div>
        )}
        ...
        {!isExpanded ? (
          ...
          <div className="space-y-4 animate-in fade-in slide-in-from-top-2 duration-200">
                ...
                {product.options.map(option => (
                  <div key={option.id} className="bg-stone-50 p-3 rounded-lg border border-stone-200">
                    <div className="flex justify-between items-center mb-2">
                       <h4 className="font-bold text-sm text-stone-800">{option.name}</h4>
                       {option.required && <span className="text-[10px] text-red-500 font-bold bg-red-50 px-1 rounded">*Obrigatório</span>}
                    </div>
                         ...
                         return (
                           <label key={idx} className="flex items-center justify-between cursor-pointer group">
                             <div className="flex items-center gap-2">
                               ...
                               } ${isSelected ? 'bg-italian-green border-italian-green' : 'bg-white border-stone-300 group-hover:border-italian-green'}`}>
                               ...
                               <input 
                                 ...
                                 checked={isSelected}
                               />
                               <span className="text-sm text-stone-700">{choice.name}</span>
                             </div>
                             {choice.price > 0 && (
                               <span className="text-xs font-semibold text-stone-500">+ R$ {choice.price.toFixed(2).replace('.',',')}</span>
                             )}
                           </label>
                         );
                      })}
            ...
            <button 
               ...
               className="w-full text-center text-xs text-stone-400 hover:text-stone-600 pb-1"
            >
